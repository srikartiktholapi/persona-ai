...
def compute_overall_score(visual_score, audio_score, text_score, rel_score, speech_detected=True, video_available=True) -> float:
    """Compute the weighted overall score.
    
    Missing scores from unavailable sections (no speech, no video) are penalized as 0.0.
    Missing scores from processing errors default to 5.0 (neutral).
    """
    # Video: penalize if unavailable, else default to 5.0 if missing
    visual_val = 0.0 if not video_available and visual_score is None else (5.0 if visual_score is None else float(visual_score))
    
    # Audio/text/relevance: penalize if speech not detected, else default to 5.0 if missing
    audio_val = 0.0 if not speech_detected and audio_score is None else (5.0 if audio_score is None else float(audio_score))
    text_val = 0.0 if not speech_detected and text_score is None else (5.0 if text_score is None else float(text_score))
    rel_val = 0.0 if not speech_detected and rel_score is None else (5.0 if rel_score is None else float(rel_score))
    
    return round(visual_val * 0.20 + audio_val * 0.25 + text_val * 0.20 + rel_val * 0.35, 2)
...
def run_batch_analysis(
    ...
    from app.agents.video import analyze_video, should_generate_visual_summary
    ...
    speech_detected       = audio_result.get("speech_detected", bool(transcript and transcript.strip()))
    ...
    results["speech_detected"]       = speech_detected
    ...
    if not isinstance(video_metrics, dict):
        video_metrics = {
            "video_analysis_available": False,
            "video_analysis_status": "Video analysis returned invalid results.",
            "face_landmarks_detected": False,
            "posture_score": None,
            "eye_contact_score": None,
            "expression_score": None,
    ...
    can_generate_visual_summary = should_generate_visual_summary(video_metrics)
    try:
        if not video_available or not can_generate_visual_summary:
            raise ValueError(
                video_metrics.get("video_analysis_status", "Video analysis unavailable for body-language summary.")
            )
    ...
    try:
        if speech_detected:
            ...
            text_result = {
                ...
                "feedback": "No clear speech was detected in the recording.",
                "text_score out of 10": None,
                "text_score": None,
            }
    except Exception as e:
    ...
    try:
        ...
        if speech_detected:
            ...
            relevance_result = {
                "relevance_score out of 50": None,
                "relevance": "unknown",
                "deductions_applied": [],
                "reason": "No speech detected, so relevance evaluation was skipped.",
            }
    except Exception as e:
    ...
    if speech_detected:
        ...
        text_score = round(max(0.0, min(10.0, float(raw_text))), 2) if raw_text is not None else None

        raw_rel = relevance_result.get("relevance_score out of 50",
                  relevance_result.get("relevance_score", None))
        rel_score = round(max(0.0, min(10.0, float(raw_rel) / 5.0)), 2) if raw_rel is not None else None
    else:
        clarity_score = confidence_score = comm_score = None
        audio_score = None
        text_score = None
        rel_score = None

    # Weighted overall: 20% video, 25% audio, 20% text, 35% relevance
    overall = compute_overall_score(visual_score, audio_score, text_score, rel_score, speech_detected, video_available)
    ...
    scores = {
        ...
        "speech_clarity_score":     round(float(clarity_score), 2) if clarity_score is not None else None,
        "confidence_score":         round(float(confidence_score), 2) if confidence_score is not None else None,
        "communication_score":      round(float(comm_score), 2) if comm_score is not None else None,
